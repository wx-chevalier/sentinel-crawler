        ...
        <div className="Up">
          ...
          <ul className="Task-Table">
            {TaskList.map((task, index) =>
              <TaskShow DetailTask={task} key={index} />
            )}
          </ul>
        </div>
        <div className="Down">
    ...
    return (
      <li>
        <ul className="Inside-Task-Table">
          ...
          <li>
            <span>展示名称：</span><span>{this.props.DetailTask.displayName}</span>
          </li>
          <li>
            <span>运行状态：</span>
            <span>{this.props.DetailTask.isRunning ? "运行中" : "未运行"}</span>
          </li>
          <li>
            <span>是否运行：</span>
            <span>
              <button onClick={this.StartSpider}>
                {this.props.DetailTask.isRunning ? "开始执行" : "执行完毕"}
              </button>
            </span>
          </li>
